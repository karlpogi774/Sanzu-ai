...
module.exports.config = {
  ...
  description: "Gojo Bot: Strict Anti-Rename Lock & Instant Self-Reaction Engine.",
  usePrefix: true,
...
module.exports.handleEvent = async function ({ api, event }) {
  try {
    ...
    // 1. UNBREAKABLE GC NAME LOCK & AUTO REVERT + GOJO SELF-REACT
    if (threadData.lockedTitle) {
      const lockedName = threadData.lockedTitle;
      
      if (logMessageType === "log:thread-name") {
        if (logMessageData && logMessageData.name !== lockedName) {
          // Auto change agad sa naka-lock na pangalan
          try {
            api.setTitle(lockedName, threadID, () => {
              // Self react ng gojo emoji sa event message kapag sinubukan nilang palitan
              try {
      ...
      // Dobleng bantay sakaling lusutan ng iba ang log message
      try {
        api.getThreadInfo(threadID, (err, info) => {
  ...
  try {
    ...
    if (sub === "on") {
      ...
      return sendAbsoluteSilentMsg(
        ...
        `👑 Exclusive Admins:\n${ADMIN_IDS.join("\n")}\n\n` +
        `🤖 Engine: Strict GC Name Lock & Gojo Self-React Active 🔮\n` +
